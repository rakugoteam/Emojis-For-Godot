...
func _enter_tree():
  rakugo = get_editor_interface().is_plugin_enabled("Rakugo")
  if rakugo:
    disable_plugin()
    return
...
func _rakugo_plugin_enabled(rakugo_menu):
  get_editor_interface().add(self)
  init_plugin()
  var icon = preload("icon.png")
  rakugo_menu.add_menu_item(menu_item[0], emoji_finder, menu_item[1], menu_item[2])
